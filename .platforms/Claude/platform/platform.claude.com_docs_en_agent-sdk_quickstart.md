...
3. **`options`**: configuration for the agent. This example uses `allowedTools` to pre-approve `Read`, `Edit`, and `Glob`, and `permissionMode: "acceptEdits"` to auto-approve file changes. Other options include `systemPrompt`, `mcpServers`, and more. See all options for [Python](https://platform.claude.com/docs/en/agent-sdk/python#claudeagentoptions) or [TypeScript](https://platform.claude.com/docs/en/agent-sdk/typescript#claudeagentoptions).
