...
Apply the active [permission mode](https://platform.claude.com/docs/en/agent-sdk/permissions#permission-modes). `bypassPermissions` approves everything that reaches this step. `acceptEdits` approves file operations. Other modes fall through.
...
If not resolved by any of the above, call your [`canUseTool` callback](https://platform.claude.com/docs/en/agent-sdk/user-input) for a decision. In `dontAsk` mode, this step is skipped and the tool is denied.


![Permission evaluation flow diagram](https://platform.claude.com/docs/images/agent-sdk/permissions-flow.svg)
