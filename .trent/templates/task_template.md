...
[ ]  -> [📋]  -> [🔄]  -> [🔍]  -> [✅]
 ^       ^        ^        ^        ^
 |       |        |        |        Completed (both TASKS.md + YAML updated atomically)
 |       |        |        Awaiting verification by a DIFFERENT agent
 |       |        In-progress (agent actively working)
 |       Spec written, task file exists, ready to implement
 Listed in TASKS.md only — NO file yet, BLOCKED, do not start
```

`[❌]` Failed — blocked permanently or abandoned after max retries.
`[⏸️]` Paused — deprioritized mid-work, preserves checkpoint state.
`harvested` — task body reused as input for a different task; original is archived.
`resource-gated` — waiting on external resource (API key, credential, hardware, human decision).
