...
├── invoke_tool() - Claude Code tool invocation
└── State management
...
- `tools` (List[str]): Claude Code tools (e.g., ["Read", "Write", "Bash"])
...
- `invoke_tool(tool_name, **kwargs) -> Any`: Invoke Claude Code tool
...
Logs are written to:
- `.claude/logs/agents/{agent_name}.log` - Agent-specific logs
- `.claude/logs/workflows/workflow.log` - Workflow orchestration logs
...
Check agent logs:
```bash
cat .claude/logs/agents/backend-developer.log
```
...
- **Fallback Handler**: `.claude/agents/sdk/fallback.py` (to be implemented)
- **Primitives Integration**: `.claude/agents/sdk/primitives/` (to be implemented)
