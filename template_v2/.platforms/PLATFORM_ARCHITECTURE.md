...
1. **Claude Code + Cursor** = Shared architecture (Skills + Agents + rules + commands). Only difference: `.md` vs `.mdc` extension and `/` vs `@` prefix
2. **Google Antigravity** = Unique (`.agent/` folder, Skills, Workflows, Artifacts, Knowledge Items)
3. **Gemini CLI** = Unique (Hierarchical GEMINI.md cascade + 3 integrated systems)
4. **Everyone Else** = Simple rules-based (no Skills/Agents)
...
| **SubAgents** | ✅ Yes (22 agents) | ❌ No | ❌ No | ✅ Yes (22 agents) | ❌ No (?) | ❌ No (?) | ❌ No (?) |
...
**3. Everyone Else (Simple Architecture)**
- **Rules-Based** - Single AI with custom instructions, no Skills/Agents
- **Directory**: `.windsurf/`, `.cline/`, `.roo/` (platform-specific)
- **File Format**: `.md`
- **Commands**: Varies by platform (TBD for most)
...
**Claude Code, Cursor, and Antigravity have Skills**

```
Claude Code: ✅ Skills + Agents + Agent SDK
  - .claude/skills/      # Knowledge modules (16 skills)
  - .claude/agents/      # Specialized AI agents (22 agents)
  - Agent SDK            # Programmatic agent creation (Python/TypeScript)

Cursor: ✅ Skills + Agents
  - .cursor/skills/      # Knowledge modules (16 skills)
  - .cursor/agents/      # Specialized AI agents (22 agents)

Antigravity: ✅ Skills only
  - .agent/skills/       # Knowledge modules (same format)

All Others: ❌
  - Must use rules/instructions instead
  - No Skills or Agent equivalent
```
...
**Workaround**:
- Keep core logic in trent (universal)
