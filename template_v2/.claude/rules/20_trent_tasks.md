...
| `[📝]` | `speccing` | Spec being written (TTL: 1h — reset to 📋 if expired) |
| `[🔄]` | `in-progress` | Actively claimed (has TTL — reset to 📋 if expired) |
| `[🔍]` | `awaiting_verification` | Impl done, pending review by DIFFERENT agent |
| `[⏳]` | `resource_gated` | Waiting on GPU/storage/API credits/hardware |
| `[✅]` | `completed` | Verified by a different agent |
| `[❌]` | `failed` | Failed or cancelled after max retries |
| `[⏸️]` | `paused` | Paused (used for phases during pivots) |
| `[🌾]` | `harvested` | Approach superseded — preserved as reference |

**ENFORCED: Cannot skip [📋] or [🔍]**
```
CORRECT: [ ] → [📋] → [🔄] → [🔍] → [✅]
