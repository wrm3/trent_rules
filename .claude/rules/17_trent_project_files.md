---
description: Rules for maintaining agents.md and CLAUDE.md project instruction files
globs: 
alwaysApply: true
---
...
- `[ ]` - Pending (no file yet) — **BLOCKED**
- `[📝]` - Agent writing spec/task file — **TTL: 1 hour** (multi-agent lock)
- `[📋]` - Ready (file created, available for pickup)
- `[🔄]` - In Progress (being coded) — **TTL: 2 hours** (multi-agent lock)
- `[✅]` - Completed
- `[❌]` - Failed/Cancelled
- `[⏸️]` - Paused/Backlog
...
- **Status flow**: [ ] → [📝] → [📋] → [🔄] → [✅]
- **Multi-agent locks**: [📝] (speccing, 1hr TTL), [🔄] (coding, 2hr TTL)
...
When `16_trent_self_improvement.mdc` detects system changes:
...
- `template/agents.md` - Minimal agents.md template
- `template/CLAUDE.md` - Minimal CLAUDE.md template (create if needed)
