...
```
.claude/memory/agent_decisions/{agent_name}/memory.json
```
...
Hooks execute bash scripts in `.claude/hooks/`:
...
```bash
#!/bin/bash
# .claude/hooks/agent-complete.sh
...
# Log completion
echo "[$(date)] Agent $AGENT completed: $TASK" >> .claude/logs/completions.log
...
```python
# .claude/agents/sdk/agents/backend_developer.py
...
- **Agent SDK Base**: `../.claude/agents/sdk/base_agent.py`
- **Context System**: `../.claude/agents/sdk/context.py`
